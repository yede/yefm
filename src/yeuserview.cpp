...
UserView::UserView(YeFilePane *pane, int type, QWidget *parent)
	: QListView(parent)
	, m_pane(pane)
	, m_type(type)
	, m_model(NULL)
...
{
	QString message = m_model->getStatusMessage(index);
	if (!message.isEmpty()) {
		int pos = m_pane->win()->pane0() == m_pane ? 0 : 1;
		m_pane->win()->showStatusMessage(message, pos, 12000);
	}
}
