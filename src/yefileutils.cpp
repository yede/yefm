...
static bool buildIconFromFile(QIcon &icon, const QString &file, int size)
{
...
static QIcon searchIconByName(const QString &name)
{
	QIcon icon;
	int size = R::data().menuIconSize;

	if (buildIconFromFile(icon, name, size)) {	// First attempt, check whether icon is a valid file
		return icon;
	}

	icon = QIcon::fromTheme(name);				// Second attempt, try load icon from theme
	if (!icon.isNull()) {
	...
	if (searchIcons.count() > 0) {
		if (buildIconFromFile(icon, "/usr/share/pixmaps/" + searchIcons.at(0), size)) {
			return icon;
		}
	}
...
QIcon FileUtils::searchAppIcon(const DesktopFile &app, const QIcon &defaultIcon)
{
	...
	QIcon icon = searchIconByName(name);
	if (!icon.isNull()) {
		return icon;
	}
...
QIcon FileUtils::searchAppIcon(const QString &name, const QIcon &defaultIcon)
{
	if (name.isEmpty()) return defaultIcon;
	...
	return searchAppIcon(df, defaultIcon);
}
//==============================================================================================================================
