...
UserTabModel::UserTabModel(FilePane *pane, QObject *parent)
	: TreeModel(parent)
	, m_pane(pane)
	, m_view(NULL)
...
//	bool sep = node->isSeparator();
	...
	switch (role) {
		...
		case Qt::DecorationRole: return IconLoader::icon("folder");
...
//	qDebug() << "UserTabModel::dropMimeData" << row << column;
	...
	if (m_dragKind == DragKind::TabItem) {
		...
		//	qDebug() << view->pane() << m_pane;
			FilePane *pane = view->pane()->pane();
			if (pane->takeTab(view)) {
