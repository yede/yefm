...
{
	return m_toolBar ? m_toolBar->itemSize() : m_defaultSize;
}
...
void ToolItem::paintEvent(QPaintEvent *event)
{
	...
	if (isOn || m_hovering)
	{
		...
		if (m_toolBar != NULL) {
			opt.rect = m_toolBar->buttonRect();
		}

		opt.state = QStyle::State_Enabled;

		if (m_toolBar != NULL && m_toolBar->checkMode() == ToolBar::cmIndicator) {
			if (m_pressed) {
				opt.state |= QStyle::State_On | QStyle::State_AutoRaise;
			} else if (m_hovering) {
...
{
	m_iconRect = m_toolBar->iconRect();
	setFixedSize(m_toolBar->itemSize());
}
//==============================================================================================================================
...
void ToolIcon::paintEvent(QPaintEvent *event)
{
	...
	if (m_toolBar != NULL && m_toolBar->checkMode() == ToolBar::cmIndicator) {
		QRect r = m_iconRect;
		if (m_pressed) {
			painter.drawPixmap(r.x() + 1, r.y() + 1, r.width(), r.height(), m_pixmap);
		} else {
			painter.drawPixmap(r, m_pixmap);
		}
		if (m_checked) {
			const QSize &btSize = m_toolBar->buttonSize();
			int W = btSize.width();
			int H = btSize.height();
			int m = m_toolBar->indicatorSize();
			int x, y, w, h;
			switch (m_toolBar->indicatorEdge()) {
				case ToolBar::ieLeft  : x = 0; y = 0; w = m; h = H; break;
				case ToolBar::ieRight : x = W; y = 0; w = m; h = H; break;
				case ToolBar::ieTop   : x = 0; y = 0; w = W; h = m; break;
				case ToolBar::ieBottom:
				default               : x = 0; y = 0; w = W; h = m; break;
			}
			QColor indicatorColor("#15b8f0");
			painter.fillRect(x, y, w, h, indicatorColor);
		}
	}
	else {
