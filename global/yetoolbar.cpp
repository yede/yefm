...
ToolBar::ToolBar(Qt::Orientation orientation, QWidget *parent)
	...
	, m_itemLayout(NULL)
	, m_tailLayout(NULL)
	, m_checkMode(cmPressDown)
	, m_indicatorEdge(ieTop)
{
}
...
void ToolBar::updateItemmSize()
{
	int padL = m_itemPads.left();
	int padR = m_itemPads.right();
	int padT = m_itemPads.top();
	int padB = m_itemPads.bottom();

	int marL = m_basePads.left();
	int marR = m_basePads.right();
	int marT = m_basePads.top();
	int marB = m_basePads.bottom();

	int icoW = m_iconSize.width();
	int icoH = m_iconSize.height();

	int w = icoW + padL + padR;
	int h = icoH + padT + padB;
	int x = padL;
	int y = padT;

	if (m_checkMode == cmIndicator) {
		int m = m_indicatorSize;
		switch (m_indicatorEdge) {
			case ieLeft  : m_buttonRect = QRect(m, 0, w, h); w += m; x += m; break;
			case ieRight : m_buttonRect = QRect(0, 0, w, h); w += m;         break;
			case ieTop   : m_buttonRect = QRect(0, m, w, h); h += m; y += m; break;
			case ieBottom:
			default      : m_buttonRect = QRect(0, 0, w, h); h += m;         break;
		}
	} else {
		m_buttonRect = QRect(0, 0, w, h);
	}

	m_iconRect = QRect(x, y, icoW, icoH);
	m_itemSize = QSize(w, h);
	m_buttonSize = m_buttonRect.size();
	setMinimumSize(w + marL + marR, h + marT + marB);
}
